...
function SkinDeformer( o )
{
	...
	//this._deformer = new RS.Deformer();
}
...
SkinDeformer["@skeleton_root_node"] = { type: RS.TYPES.SCENENODE };
...
SkinDeformer.prototype.onCollectInstances = function( e, render_instances )
{
	...
	if(!last_RI){
		prev_comp = this.root.getComponent(RS.MeshRenderer);
		if(prev_comp){
			last_RI = prev_comp._RI;
		}

		if(!last_RI){
			prev_comp = this.root.getComponent(RS.SkinnedMeshRenderer);
			if(prev_comp){
				last_RI = prev_comp._RI;
			}
		}
		...
		if(!last_RI){
			console.warn("can not found MeshRenderer/SkinnedMeshRenderer component");
			return;
		}
	}
...
{
	if(!RI.mesh){
		if(RI.component._RIs && RI.component._RIs.length > 0){
			for(RI of RI.component._RIs && RI.component._RIs){
				let mesh = RI.mesh;
				this._mesh = mesh;
				...
				if( SkinDeformer.gpu_skinning_supported && !this.cpu_skinning )
				{
					...
					if( SkinDeformer.num_supported_uniforms >= bones_size )
					{
						...
						RI.samplers[ RS.Renderer.BONES_TEXTURE_SLOT ] = null;

						RI.addShaderBlock( RS.SkinDeformer.skinning_uniforms_block, { u_bones: u_bones } );
					}
					else if( SkinDeformer.num_supported_textures > 0 ) //upload the bones as a float texture (slower)
					{
						...
						RS.RM.textures[":bones_" + this.uid ] = texture; //debug
						RI.uniforms["u_bones"] = RS.Renderer.BONES_TEXTURE_SLOT;
						RI.query.macros["USE_SKINNING_TEXTURE"] = "";
						RI.samplers[ RS.Renderer.BONES_TEXTURE_SLOT ] = texture; //{ texture: texture, magFilter: gl.NEAREST, minFilter: gl.NEAREST, wrap: gl.CLAMP_TO_EDGE };

						RI.addShaderBlock( RS.SkinDeformer.skinning_texture_block, { u_bones: RS.Renderer.BONES_TEXTURE_SLOT } );
					}
					else
						console.error("impossible to get here");

					RI.addShaderBlock( RS.SkinDeformer.skinning_block );
				}
				...
				mat4.multiplyVec3( RI.center, RI.matrix, RS.ZEROS );
...
SkinDeformer.prototype.disableSkinning = function( RI )
{
	...
	RI.removeShaderBlock( RS.SkinDeformer.skinning_block );
	RI.removeShaderBlock( RS.SkinDeformer.skinning_uniforms_block );
	RI.removeShaderBlock( RS.SkinDeformer.skinning_texture_block );
}
...
RS.registerComponent( SkinDeformer );
RS.SkinDeformer = SkinDeformer;
...
var skinning_block = new RS.ShaderBlock("skinning");
...
var skinning_uniforms_block = new RS.ShaderBlock("skinning_uniforms");
...
var skinning_texture_block = new RS.ShaderBlock("skinning_texture");
