...
RS.Formats = {
	...
	*/
	parse: async function( filename, data, options)
	{
		options = options || {};
		...
		if(options.extension)
			...
			info.extension = RS.ResourcesManager.getExtension( filename );
		...
		if(!this.safe_parsing)
			result = await format.parse( data, options, filename );
		else
			try
			{
				result = await format.parse( data, options, filename );
	...
	guessType: function( filename )
	{
		...
		var ext = RS.RM.getExtension( filename ).toLowerCase();
...
RS.Formats.addSupportedFormat( "png,jpg,jpeg,webp,bmp,gif", { "native": true, dataType: "arraybuffer", resource: "Texture", "resourceClass": GL.Texture, has_preview: true, type: "image" } );
RS.Formats.addSupportedFormat( "wbin", { dataType: "arraybuffer" } );
RS.Formats.addSupportedFormat( "json,js,ts,txt,html,css,csv", { dataType: "text" } );//add typescript support
RS.Formats.addSupportedFormat( "glsl", { dataType: "text", resource: "ShaderCode", "resourceClass": RS.ShaderCode  } );
RS.Formats.addSupportedFormat( "zip", { dataType: "arraybuffer" } );
WBin.classes = RS.Classes; //WBin need to know which classes are accesible to be instantiated right from the WBin data info, in case the class is not a global class
