...
function MeshRenderer(o)
{
	...
	*/
	...
	this._RI = new RS.RenderInstance( null, this );
	//this._RIs = [];
...
MeshRenderer["@use_submaterials"] = { type: RS.TYPES.BOOLEAN, widget: null }; //avoid widget
...
MeshRenderer.prototype.onAddedToNode = function( node )
{
	...
	var mesh = RS.ResourcesManager.getMesh( this._mesh );
	if(!mesh || !mesh.info || !mesh.info.groups || mesh.info.groups.length < 1)
		return null;
	this.use_submaterials = true;
	for(let i=0; i<mesh.info.groups.length; i++){
		this.submaterials[i] = mesh.info.groups[i] ? mesh.info.groups[i].material : "";
	}
}
...
MeshRenderer.prototype.getMesh = function() {
	...
	if( this.mesh.constructor === String )
		return RS.ResourcesManager.meshes[ this.mesh ];
	return this.mesh;
}
...
MeshRenderer.prototype.getLODMesh = function() {
	...
	if( this.lod_mesh.constructor === String )
		return RS.ResourcesManager.meshes[ this.lod_mesh ];
...
MeshRenderer.prototype.getResources = function(res)
{
	...
	if( this.material && this.material.constructor === String )
		res[this.material] = RS.Material;

	if(this.use_submaterials)
	{
		for(var i  = 0; i < this.submaterials.length; ++i)
			if(this.submaterials[i])
				res[this.submaterials[i]] = RS.Material;
	}
	return res;
}
...
MeshRenderer.prototype.updateRIs = function()
{
	...
	//optimize: TODO
	//if( is_static && RS.allow_static && !this._must_update_static && (!transform || (transform && this._transform_version == transform._version)) )
	...
	var material = null;
	if(this.material)
		material = RS.ResourcesManager.getResource( this.material );
	else
	...
	else
	{
		...
		if(this._once_binding_index != null)
			this._once_binding_index = RS.ResourcesManager.onceLoaded( this._mesh, this.updateRIs.bind(this ) );
	}
	...
	{
		if( this.lod_mesh.constructor === String )
			RI.collision_mesh = RS.ResourcesManager.resources[ this.lod_mesh ];
		else
	...
	*/
	...
	if( is_static && RS.allow_static && !this.isLoading() )
	{
...
MeshRenderer.prototype.onCollectInstancesSubmaterials = function(instances)
{
	...
	mat4.multiplyVec3( center, global, RS.ZEROS );
	var first_RI = null;
	...
	for(var i = 0; i < this.submaterials.length; ++i)
	{
		...
		var material = RS.ResourcesManager.getResource( submaterial_name );
		if(!material){
			continue;
		}
		...
		var RI = this._RIs[i];
		if(!RI)
			RI = this._RIs[i] = new RS.RenderInstance(this._root,this);
...
{
	if( this.mesh && RS.ResourcesManager.isLoading( this.mesh ))
		return true;
	if( this.lod_mesh && RS.ResourcesManager.isLoading( this.lod_mesh ))
		return true;
	if( this.material && RS.ResourcesManager.isLoading( this.material ))
		return true;
	if(this._root && this._root.material && this._root.material.constructor === String && RS.ResourcesManager.isLoading( this._root.material ))
		return true;
	return false;
}
...
MeshRenderer.prototype.explodeSubmeshesToChildNodes = function() {
	...
	for(var i = 0; i < mesh.info.groups.length; ++i)
	{
		...
		var child_node = new RS.SceneNode();
		node.addChild( child_node );
		var comp = new RS.Components.MeshRenderer({ mesh: this.mesh, submesh_id: i, material: group.material });
...
RS.registerComponent( MeshRenderer );
RS.MeshRenderer = MeshRenderer;
