...
var ResourcesManager = {
	...
	*/
	getResource: function( url, constructor )
	{
		...
		url = LFSBridge.getResourceFullPath(url);
	...
	*/
	getResourceType: function( resource )
	{
		...
		return RS.getObjectClassName( resource );
	},
	...
	*/
	getResourcesData: function( resource_names, allow_files )
	{
		...
		for(var i = 0; i < resource_names.length; ++i)
		{
			...
			var resource = RS.ResourcesManager.resources[ res_name ];
			if(!resource)
				continue;
			...
			{
				var data_info = RS.Resource.getDataToStore( resource );
				data = data_info.data;
			}
	...
	*/
	resourceModified: function( resource )
	{
		...
		if(resource.from_pack)
		{
			...
			{
				var pack = RS.ResourcesManager.getResource( resource.from_pack );
				if(pack)
		...
		if(resource.from_prefab)
		{
			...
			{
				var prefab = RS.ResourcesManager.getResource( resource.from_prefab );
				if(prefab)
	...
	* Do not use to load regular files (txts, csv, etc), instead use the RS.Network methods
	*
	* @method load
	* @param {String} url where the resource is located (if its a relative url it depends on the path attribute)
	...
	*/
	...
	{
		if(!url)
			return console.error("RS.ResourcesManager.load requires url");
		...
		if(!this.allow_base_files && url.indexOf("/") == -1)
		{
			if(!this._parsing_local_file) //to avoid showing this warning when parsing scenes with local resources
				console.warn("Cannot load resource, filename has no folder and RS.ResourcesManager.allow_base_files is set to false: ", url );
			//cwx: return; //this is not a valid file to load

			//cw: if we are "parsing a local file", we add in the folder or it will fail!
			if(this._parsing_local_file) //to avoid showing this warning when parsing scenes with local resources
			{
				if (typeof resource_base_path==="undefined")
				{
					console.log("cw: illegal sub-resource filename, but we shouldn't be loading anyway here..:"+url)
				}
				else
				{
					// cw:make it lower case, this might break sth but we'll have to fix that as we find it
					url = url.toLowerCase();

					url = resource_base_path+url;
					console.log("cw: caught file without folder (assuming relative and fixing to):"+url);
				}
			}
			else
		...
		LEvent.trigger( RS.ResourcesManager, "resource_loading", url );
		//send an event if we are starting to load (used for loading icons)
		//if(this.num_resources_being_loaded == 0)
		//	LEvent.trigger( RS.ResourcesManager,"start_loading_resources", url );
		...
		var settings = {
			...
			success: function(response){
				RS.ResourcesManager.processResource( url, response, options, RS.ResourcesManager._resourceLoadedEnd, true );
			},
			error: function(err) {
				RS.ResourcesManager._resourceLoadedError(url,err);
				if(on_error)
					on_error(url);
			},
			progress: function(e) {
				...
				if( LEvent.hasBind(  RS.ResourcesManager, "resource_loading_progress" ) ) //used to avoid creating objects during loading
					LEvent.trigger( RS.ResourcesManager, "resource_loading_progress", { url: url, event: e, progress: partial_load } );
				if( LEvent.hasBind(  RS.ResourcesManager, "loading_resources_progress" ) ) //used to avoid creating objects during loading
					LEvent.trigger( RS.ResourcesManager, "loading_resources_progress", 1.0 - (RS.ResourcesManager.num_resources_being_loaded - partial_load) / RS.ResourcesManager._total_resources_to_load );
			}
		};
		...
		var format_info = RS.Formats.supported[ extension ];
		if( format_info )
		{
		...
		//send the REQUEST
		RS.Network.request( settings ); //ajax call
		return false;
	},
	...
	*/
	processResource: async function( url, data, options, on_complete, was_loaded )
	{
		options = options || {};
		...
		if(extension)
			format_info = RS.Formats.supported[ extension ];
		...
		var process_final = function( url, resource, options ){
			if(!resource)
			{
				RS.ResourcesManager._resourceLoadedEnd( url, null ); //to remove it from loading
				return;
			}
			...
			//apply last changes
			RS.ResourcesManager.processFinalResource( url, resource, options, on_complete, was_loaded );

			//Keep original file inside the resource in case we want to save it
			if(RS.ResourcesManager.keep_files && (data.constructor == ArrayBuffer || data.constructor == String) && (!resource._original_data && !resource._original_file) )
			{
				if( extension == RS.ResourcesManager.getExtension( resource.filename ) )
		...
		if(!extension || extension=="txt")	// cw: added "txt"
		...
		else if( format_info && (format_info.type || format_info.parse) ) //or you can rely on the format info parser
		{
			...
			{
				case "scene":
					resource = await RS.ResourcesManager.processScene( url, data, options, process_final );
					break;
				case "mesh":
					resource = RS.ResourcesManager.processTextMesh( url, data, options, process_final );
					break;
				case "texture":
				case "image":
					resource = RS.ResourcesManager.processImage( url, data, options, process_final );
					break;
				case "data":
				default:
		...
		else //or just store the resource as a plain data buffer
		{
			...
			else //otherwise create a generic RS.Resource (they store data or scripts)
			{
				//if we already have a RS.Resource, reuse it (this is to avoid garbage and solve a problem with the editor
				var old_res = this.resources[url];
				if( old_res && old_res.constructor === RS.Resource )
				{
					resource = old_res;
					delete resource._original_data;
					delete resource._original_file;
	...
	*/
	processFinalResource: function( fullpath, resource, options, on_complete, was_loaded )
	{
		if(!resource || resource.constructor === String)
			return RS.ResourcesManager._resourceLoadedError( fullpath, "error processing the resource" );
		...
		if( RS.ResourcesManager.resources_being_processed[ fullpath ] )
			delete RS.ResourcesManager.resources_being_processed[ fullpath ];

		//Load associated resources (some resources like RS.Prefab or RS.SceneTree have other resources associated that must be loaded too)
		if( resource.getResources )
			RS.ResourcesManager.loadResources( resource.getResources({}) );

		//REGISTER adds to containers *******************************************
		RS.ResourcesManager.registerResource( fullpath, resource );
		if(options.preview_of)
			RS.ResourcesManager.registerResource( options.preview_of, resource );
	...
	*/
	registerResource: function( filename, resource )
	{
		...
		if(!resource.object_class)
			resource.object_class = RS.getObjectClassName( resource );
		var type = resource.object_class;
		if(resource.constructor.resource_type)
			type = resource.constructor.resource_type;

		if(type == 'Mesh' || type == 'Animation') {
			var ext = RS.RM.getExtension( resource.filename );
			if(!ext) {
				filename = resource.filename + '.wbin';
			}
		}

		var folder = RS.RM.getFolder(filename);
		if(!folder) {//internal parsed data
			filename = LFSBridge.getDefaultProjectDirName() + '/' + filename;
			resource.filepath = resource.filename = resource.fullpath = filename;
		}
		...
		RS.GlobalScene.requestFrame(); //render scene
	},
	...
	*/
	unregisterResource: function(filename)
	{
		...
		if(resource.constructor === RS.Pack || resource.constructor === RS.Prefab)
	...
	*/
	renameResource: function( old_name, new_name, skip_event )
	{
		...
		if(!skip_event)
			RS.GlobalScene.sendResourceRenamedEvent( old_name, new_name, res );
		...
		if(!skip_event)
			LEvent.trigger( RS.ResourcesManager, "resource_renamed", [ old_name, new_name, res ] );
		return true;
	},
	...
	*/
	...
	{
		if( RS.ResourcesManager.debug )
			console.log("RES: " + url + " ---> " + RS.ResourcesManager.num_resources_being_loaded);

		if(res)
		{
			//trigger all associated load callbacks
			var callbacks_array = RS.ResourcesManager.resources_being_loaded[url];
			if(callbacks_array)
			...
			var callbacks_array = RS.ResourcesManager.resource_once_callbacks[url];
			if(callbacks_array)
			{
				for(var i = 0; i < callbacks_array.length; ++i)
					if(callbacks_array[i]) //could be null if it has been canceled
						callbacks_array[i](url, res);
				delete RS.ResourcesManager.resource_once_callbacks[url];
			}
		}

		//two pases, one for launching, one for removing
		if( RS.ResourcesManager.resources_being_loaded[url] )
		{
			delete RS.ResourcesManager.resources_being_loaded[url];
			RS.ResourcesManager.num_resources_being_loaded--;
			if(res)
				LEvent.trigger( RS.ResourcesManager, "resource_loaded", url );
			else
				LEvent.trigger( RS.ResourcesManager, "resource_problem_loading", url );
			LEvent.trigger( RS.ResourcesManager, "loading_resources_progress", 1.0 - RS.ResourcesManager.num_resources_being_loaded / RS.ResourcesManager._total_resources_to_load );
			//---------------------------------------------------------------
			// cw: check for all resources loaded here... if they are all done, we can do any saving of mem resources...
			//---------------------------------------------------------------
			if( RS.ResourcesManager.num_resources_being_loaded == 0)
			{
				LEvent.trigger( RS.ResourcesManager, "end_loading_resources", true);

				if (CollaborateModule.collaborating)
				{
					//cw: here we can autosave any resources
					DriveModule.autoSaveMissingResources(function()
					{
						// Comes here when all have been uploaded. @cwtodo I think this is actually when it has STARTED all uploads...hmm
						//RealGUI.alert("saved memory resources to "+RS.GlobalScene.extra.folder);
						CollaborateModule.onUserAction( [] );	// Tell others in collab mode the new scene graph, they will autoload resources.
					});
				}
				RS.ResourcesManager._total_resources_to_load = 0;
			}
		}

		//request frame
		RS.GlobalScene.requestFrame();
	},
	...
	_resourceLoadedError: function( url, error )
	{
		...
		delete RS.ResourcesManager.resources_being_loaded[url];
		delete RS.ResourcesManager.resource_once_callbacks[url];
		RS.ResourcesManager.resources_not_found[url] = true;
		LEvent.trigger( RS.ResourcesManager, "resource_not_found", url);
		RS.ResourcesManager.num_resources_being_loaded--;
		if( RS.ResourcesManager.num_resources_being_loaded == 0 )
		{
...
RS.RM = RS.ResourcesManager = ResourcesManager;

RS.getTexture = function( name_or_texture ) {
	return RS.ResourcesManager.getTexture( name_or_texture );
}


// Resources readers and processors *********************************************
// When loading resources there are two stages:
// * Pre-process: extract from a container, parse raw data and transform it in a RS resource class (Texture,Mesh,SceneNode,Resource, ...)
...
RS.ResourcesManager.registerResourcePreProcessor("wbin", function( filename, data, options) {
	...
	{
		if( RS.Classes[ data.object_class ] )
		{
			var ctor = RS.Classes[ data.object_class ] || window[ data.object_class ];
...
RS.ResourcesManager.registerResourcePreProcessor("json", function(filename, data, options) {
	var resource = data;
	...
	{
		var ctor = RS.Classes[ class_name ] || window[ class_name ];
		if(ctor)
		{
...
//global formats: take a file and extract info
RS.ResourcesManager.registerResourcePreProcessor("zip", function( filename, data, options ) {
	...
	zip.loadAsync( data ).then(function(zip){
		zip.forEach(function (relativePath, file){
			...
			var ext = RS.ResourcesManager.getExtension( relativePath );
			var format = RS.Formats.supported[ ext ];
			file.async( format && format.dataType == "text" ? "string" : "arraybuffer").then( function(filedata){
				if( relativePath == "scene.json" && (!options || !options.to_memory) )
					RS.GlobalScene.configure( JSON.parse( filedata ) );
				else
					RS.ResourcesManager.processResource( relativePath, filedata );
			});
		});
	});
...
RS.ResourcesManager.processDataResource = function( url, data, options, callback )
{
	//JSON?
	if( data.constructor === String )
		data = JSON.parse(data);
	...
	if(class_name && RS.Classes[class_name] )
	{
		var ctor = RS.Classes[class_name];
		var resource = null;
		if(ctor.prototype.configure)
		{
			resource = new RS.Classes[class_name]();
			resource.configure( data );
		}
		else
			resource = new RS.Classes[class_name]( data );
		if(callback)
			callback(url, resource, options);
		return resource;
	}
...
RS.ResourcesManager.processImage = function( filename, data, options, callback ) {

	var extension = RS.ResourcesManager.getExtension(filename);
	...
	if(extension == "jpg" || extension == "jpeg")
		...
		var format = RS.Formats.supported[ extension ];
		if(format.mimetype)
	...
	image.onload = function()
	{
		...
		var texture = RS.ResourcesManager.processTexture( filename, this, options );
		inner_on_texture( texture );
	}
	...
	function inner_on_texture( texture )
	{
		if(texture)
		{
			//RS.ResourcesManager.registerResource( filename, texture ); //this is done already by processResource
			if(RS.ResourcesManager.keep_files)
				texture._original_data = data;
		}

		if( !RS.ResourcesManager.keep_urls )
			URL.revokeObjectURL( objectURL ); //free memory
		else
...
RS.ResourcesManager.processImageNonNative = function( filename, data, options ) {

	//clone because DDS changes the original data
	var cloned_data = new Uint8Array(data).buffer;
	var texture_data = RS.Formats.parse( filename, cloned_data, options );
	...
	var texture = RS.ResourcesManager.processTexture( filename, texture_data );
	return texture;
}

//Takes one image (or canvas or object with width,height,pixels) as input and creates a GL.Texture
RS.ResourcesManager.processTexture = function(filename, img, options)
{
...
RS.ResourcesManager.processTextMesh = function( filename, data, options ) {

	var mesh_data = RS.Formats.parse( filename, data, options );
...
RS.ResourcesManager.processScene = async function( filename, data, options ) {
	//options = options || {};

	var scene_data = await RS.Formats.parse( filename, data, options );
	...
	for(var i in scene_data.resources)
	{
		...
		RS.ResourcesManager.processResource(i,res);
	}

	for(var i in scene_data.materials)
	{
		var material = scene_data.materials[i];
		RS.ResourcesManager.processResource(i,material);
	}

	var node = new RS.SceneNode();
	node.configure( scene_data.root );

	//make it a pack or prefab
	if(options && options.filename)
	{
		var ext = RS.RM.getExtension( options.filename );
		if(ext != "json")
			options.filename += ".json";
	}

	RS.ResourcesManager._parsing_local_file = false;
...
RS.ResourcesManager.registerResourcePostProcessor("Mesh", function(filename, mesh ) {
	...
	if(RS.ResourcesManager.free_data) //free buffers to reduce memory usage
		mesh.freeData();

	RS.ResourcesManager.meshes[filename] = mesh;
});

RS.ResourcesManager.registerResourcePostProcessor("Texture", function( filename, texture ) {
	//store in appropiate container
	RS.ResourcesManager.textures[filename] = texture;
});

RS.ResourcesManager.registerResourcePostProcessor("Material", function( filename, material ) {
	//store in appropiate containers
	RS.ResourcesManager.materials[filename] = material;
	RS.ResourcesManager.materials_by_uid[ material.uid ] = material;
	if(material.prepare)
		material.prepare( RS.GlobalScene );
});

RS.ResourcesManager.registerResourcePostProcessor("Pack", function( filename, pack ) {
	//flag contents to specify where do they come from
	pack.flagResources();
});

RS.ResourcesManager.registerResourcePostProcessor("Prefab", function( filename, prefab ) {
	//apply to nodes in the scene that use this prefab
	prefab.applyToNodes();
});

RS.ResourcesManager.registerResourcePostProcessor("ShaderCode", function( filename, shader_code ) {
