...
global.Collada = {
	...
	parse: function(data, options, filename)
	{
		...
		if(animations)
		{
			//cwx var animations_name = "#animations_" + filename.substr(0,filename.indexOf("."));
				...
				if(xmlmaterials)
				{
					for(var iMat = 0; iMat < xmlmaterials.length; ++iMat)
					{
						...
						if(!scene.materials[ matname ])
						{
							...
							if(material)
							{
								matname = matname.replace(/[^a-z0-9\.\-]/gi,"-");	//cw: added
								material.id = matname;
	...
	readPolygons: function( xmlpolygons, sources )
	{
		...
		for(var i = 0; i < xmlp_array.length; i++)
			...
			//change the material id due to post processing(cw's)
			if(material_name != ""){
				material_name = material_name.replace(/[^a-z0-9\.\-]/gi,"-") + ".json";
			}
	...
	readImages: function(root)
	{
		...
		for(var i = 0; i < xmlimages_childs.length; ++i)
		{
			...
			if(xmlinitfrom.textContent)
			{
				...
				filename = filename.toLowerCase();	//cw: added

				var id = xmlimage.getAttribute("id");
				//cwx images[id] = { filename: filename, map: id, name: xmlimage.getAttribute("name"), path: xmlinitfrom.textContent };
				images[id] = { filename: filename, map: id, name: xmlimage.getAttribute("name"), path: xmlinitfrom.textContent.toLowerCase() };	//cw: added tolowerCase

				//cw: fixup if these are RELATIVE to a base asset we're importing...
				if(true)
				{
					images[id].filename =resource_base_path+images[id].filename;
					images[id].path =resource_base_path+images[id].path;
			}
		}
		}
