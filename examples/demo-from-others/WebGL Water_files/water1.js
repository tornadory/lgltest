function Water() {
    ...
    if (!GL.Texture.canUseFloatingPointTextures()) {
        throw new Error("This demo requires the OES_texture_float_extension");
    }
    ...
    this.updateShader = new GL.Shader(vertexShader, `
        ...
        void main() {
            /* get vertex info */
            vec4 info = texture2D(texture, coord);
            /* calculate average nerghbor height */
    ...
    this.normalShader = new GL.Shader(vertexShader, `
        ...
        void main() {
            ...
            vec4 info = texture2D(texture, coord);
            /* update the normal */
            vec3 dx = vec3(delta.x, texture2D(texture, vec2(coord.x + delta.x, coord.y)).r - 
                info.r, 0.0);
            vec3 dy = vec3(0.0, texture2D(texture, vec2(coord.x, coord.y + delta.y)).r - 
                info.r, delta.y);
